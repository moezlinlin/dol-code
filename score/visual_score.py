# 每个子文件夹下创建一个 annotated/ 目录，把在图像上用绿色画真实中心、用红色画预测中心后的结果保存进去
# -*- coding: utf-8 -*-
import os
import cv2
import time
...
def process_single_folder(folder_path):
    ...
    annot_folder = os.path.join(folder_path, "annotated")     # 新增：保存可视化图像
    ...
    for idx, fname in enumerate(image_files, start=1):
        ...
        # —— 可视化：在图像上画出真实中心（绿）和预测中心（红） ——
        vis = img.copy()
        gt_pt  = (int(gt_center[0]),  int(gt_center[1]))
        pred_pt= (int(pred_center[0]), int(pred_center[1]))
        cv2.circle(vis, gt_pt,   radius=5, color=(0,255,0), thickness=-1)  # 真实中心：绿色
        cv2.circle(vis, pred_pt, radius=5, color=(0,0,255), thickness=-1)  # 预测中心：红色
        cv2.putText(vis, f"{idx}", (10,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
        cv2.imwrite(os.path.join(annot_folder, fname), vis)
        ...
        # print(f"{fname}: score={total_score:.4f}, acc={acc_score:.1f}, "
        #       f"time={elapsed:.1f}ms, fps={fps:.1f}, "
        #       f"w={w_box:.1f}, h={h_box:.1f}")
